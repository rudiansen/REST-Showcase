...
//			clientApp.clearVariables();
			...
			height = new LifeStatus();
			height.setMeasureDefinition(new MeasureDefinition(2));
			height.setValue("172");
			
			lifeStatusList = new ArrayList<LifeStatus>();
			lifeStatusList.add(weight);
			lifeStatusList.add(height);
			
			newPerson = new Person(lifeStatusList);
			newPerson.setIdPerson(newIdPerson);
			newPerson.setName("Chuck");
			newPerson.setLastname("Norris");
			newPerson.setBirthdate("1945-01-01");
...
//			clientApp.clearVariables();
			 ...
			 */	
			//Get HealthMeasureStore based on provided mid
			response = service.path("person/"+storedIdPerson+"/"+storedMeasureType+"/"+mid).request()
			...
			//Un-marshall HealthMeasureHistory xml String into HealthMeasureHistory object
			healthMeasureHsitory = writer.unmarshallJSONHealthMeasure(stream);
			if(httpStatus == 200 || httpStatus == 201)
				resultStatus = "OK";
			else
				resultStatus = "ERROR";
			
			printHeader(7, "GET", "/person/"+storedIdPerson+"/"+storedMeasureType+"/"+mid, "APPLICATION/JSON", "APPLICATION/JSON");		
			System.out.println("=> Result: "+ resultStatus);
			System.out.println("=> HTTP Status: "+ httpStatus);
			
			//Marshall HealthMeasureHistory object into the system default output
			writer.marshallJSON(healthMeasureHsitory);
			System.out.println("\n");
			clientApp.clearVariables();
			...
			//Un-marshall HealthMeasureHistory xml String into HealthMeasureStore object
			healthMeasureList = writer.unmarshallJSONHealthMeasureList(stream);
			countHealthMeasure = healthMeasureList.length;
			...
			printHeader(6, "GET", "/person/"+first_person_id+"/"+measure_types[0], "APPLICATION/JSON", "APPLICATION/JSON");		
			System.out.println("=> Result: "+ resultStatus);
			System.out.println("=> HTTP Status: "+ httpStatus);
...
//			clientApp.clearVariables();
			...
			healthMeasureList = writer.unmarshallJSONHealthMeasureList(stream);
			//Check if the HealthMeasureHistory is more than the previous one
			if(healthMeasureList.length > countHealthMeasure)
				resultStatus = "OK";
			else
				resultStatus = "ERROR";
